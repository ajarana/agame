...
function positionBlocks() {
  ...
  for (var i=0; i < numberOfRows; i++) {
    for (var j=0; j < numberOfColumns; j++) {
      var x = xCenterOfCanvas + 3*j + (xCoefficient*blockLength);
      var y = yCenterOfCanvas + 3*i + (yCoefficient*blockLength);
...
function setDimensions(event) {
  ...
  if (window.innerHeight > window.innerWidth) {
    if (window.innerWidth > 768) {
      ...
      var width = Math.round(0.99 * window.innerWidth);
    ...
    if (window.innerHeight > 768) {
      ...
      var width = Math.round(0.99 * window.innerHeight);
      blockFeedbackText.style.fontSize = "140%";
    }
  }
  ...
  blockLength = Math.round((canvas.width/numberOfRows));
